...
import { searchPokemonCards, getFeaturedPokemonCards, searchScryfallCards, searchTCGDexCards, searchMercadoLivre, searchPokeTrace, searchCardMarketCards, getCardMarketEpisodeCards, calcMLStats, searchYuGiOhCards, searchOnePieceCards, searchLorcanaCards, type PokemonCard, type ScryfallCard, type TCGDexCard, type CardMarketTCGCard, type YuGiOhCard, type OnePieceCard, type LorcanaCard } from "@/lib/api";
...
export function useCardMarketEpisode(episodeId:number|null,game:"pokemon"|"magic"|"lorcana"|"star-wars"="pokemon"){return useQuery<CardMarketTCGCard[]>({queryKey:["cardmarket",game,"episode",episodeId],queryFn:()=>getCardMarketEpisodeCards(episodeId!,game),enabled:episodeId!=null,staleTime:15*60*1000,gcTime:GC,retry:1});}

export function useYuGiOhCards(query:string){const q=useDebounce(query.trim(),400);return useQuery<YuGiOhCard[]>({queryKey:["yugioh","search",q],queryFn:()=>searchYuGiOhCards(q),enabled:q.length>=2,staleTime:STALE,gcTime:GC,retry:2});}

export function useOnePieceCards(query:string){const q=useDebounce(query.trim(),400);return useQuery<OnePieceCard[]>({queryKey:["onepiece","search",q],queryFn:()=>searchOnePieceCards(q),enabled:q.length>=2,staleTime:STALE,gcTime:GC,retry:1});}

export function useLorcanaCards(query:string){const q=useDebounce(query.trim(),400);return useQuery<LorcanaCard[]>({queryKey:["lorcana","search",q],queryFn:()=>searchLorcanaCards(q),enabled:q.length>=2,staleTime:STALE,gcTime:GC,retry:1});}
